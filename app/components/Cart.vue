...
const togglePromoCode = () => {
  ...
  // Here you could add logic to apply/remove the actual discount
}
...
// Similar products - load products aligned with the user's cart
...
const loadSimilarProducts = async () => {
  ...
  try {
      ...
      uniqueProductIds.map(async productId => {
        try {
          ...
          if (productData?.category_id) {
            ...
            const categoryId = Number(productData.category.id)
            if (Number.isFinite(categoryId)) {
              categoryIds.add(categoryId)
            }
          }

          if (typeof productData?.bmw_hauptgruppe === 'string' && productData.bmw_hauptgruppe.trim() !== '') {
            hauptgruppen.add(productData.bmw_hauptgruppe.trim())
          }
          
          const compatibleModelIds = Array.isArray(productData?.compatible_models?.model_ids)
            ? productData.compatible_models.model_ids
          ...
          const compatibleCodes = Array.isArray(productData?.compatible_models?.codes)
            ? productData.compatible_models.codes
          ...
          console.error(`Failed to load product ${productId} for related suggestions`, error)
        }
      })
    )
    
    if (categoryIds.size === 0 && modelIds.size === 0 && modelCodes.size === 0 && hauptgroepen.size === 0) {
      similarProducts.value = []
      return
    }
    ...
    const relatedProducts = await productService.getRelatedProducts({
      ...
      hauptgruppen: Array.from(hauptgroepen),
            ...
            <div class="mb-6">
              <div class="relative">
                ...
                <button 
                  ...
                  class="absolute right-2 top-1/2 transform -translate-y-1/2 w-6 h-6 border border-gray-300 rounded flex items-center justify-center hover:bg-gray-50 cursor-pointer"
                >
