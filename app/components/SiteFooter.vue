<script setup lang="ts">
import { useLocale } from '../stores/locale'

const { t } = useLocale()
</script>

<template>
  <footer class="mt-10 bg-white">
    <div class="container mx-auto px-6 md:px-10 py-12">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
        <!-- Left Section: Logo & Description -->
        <div class="md:col-span-1">
          <img src="/images/bimmerparts-logo-new.png" alt="BIMMERParts" class="h-10 w-auto object-contain mb-4" />
          <p class="text-gray-600 text-sm mb-6">{{ t('footer.description') }}</p>
          <NuxtLink to="/contact" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 text-sm hover:bg-gray-50 transition-colors inline-block">
            {{ t('footer.contact') }}
          </NuxtLink>
        </div>

        <!-- Middle Section: Navigation Links -->
        <div class="md:col-span-2 grid grid-cols-2 gap-8">
          <!-- About Column -->
          <div>
            <h4 class="font-semibold text-gray-900 mb-4">About</h4>
            <ul class="space-y-3 text-gray-600 text-sm">
              <li><NuxtLink to="/about" class="hover:text-gray-900 transition-colors">About us</NuxtLink></li>
              <li><a href="#" class="hover:text-gray-900 transition-colors">Features</a></li>
              <li><a href="#" class="hover:text-gray-900 transition-colors">Blog</a></li>
              <li><a href="#" class="hover:text-gray-900 transition-colors">Download</a></li>
            </ul>
          </div>

          <!-- Company Column -->
          <div>
            <h4 class="font-semibold text-gray-900 mb-4">Company</h4>
            <ul class="space-y-3 text-gray-600 text-sm">
              <li><a href="#" class="hover:text-gray-900 transition-colors">How we work</a></li>
              <li><a href="#" class="hover:text-gray-900 transition-colors">Press Room</a></li>
              <li><a href="#" class="hover:text-gray-900 transition-colors">Jobs</a></li>
              <li><a href="#" class="hover:text-gray-900 transition-colors">Community</a></li>
            </ul>
          </div>
        </div>

        <!-- Right Section: Contact -->
        <div class="md:col-span-1">
          <div class="space-y-3 mb-6">
            <div class="flex items-center gap-2 text-gray-600 text-sm">
              <img src="/images/location.png" alt="Location" class="w-4 h-4" />
              <span>Bimmer Parts - Amsterdam</span>
            </div>
            <div class="flex items-center gap-2 text-gray-600 text-sm">
              <img src="/images/sms.png" alt="Email" class="w-4 h-4" />
              <span>contact@bimmer.nl</span>
            </div>
            <div class="flex items-center gap-2 text-gray-600 text-sm">
              <img src="/images/Phone.png" alt="Phone" class="w-4 h-4" />
              <span>+383 49 324 009</span>
            </div>
          </div>

          <div class="mb-6 flex items-center gap-3">
            <p class="text-gray-600 text-sm">We accept:</p>
            <img src="/images/visa.png" alt="Visa" class="h-6" />
            <img src="/images/mastercard.png" alt="Mastercard" class="h-6" />
          </div>

          <div class="flex items-center gap-3">
            <p class="text-gray-600 text-sm">Follow us:</p>
            <a href="#" class="text-gray-600 hover:text-blue-600 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
              </svg>
            </a>
            <a href="#" class="text-gray-600 hover:text-pink-600 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
              </svg>
            </a>
          </div>
        </div>
      </div>
    </div>

    <div class="border-t py-4 text-center text-sm text-gray-500">
      Copyright © 2025 Bimmer Parts |
      <NuxtLink to="/terms" class="hover:text-gray-700 transition-colors">{{ t('footer.terms') }}</NuxtLink> |
      <NuxtLink to="/privacy" class="hover:text-gray-700 transition-colors">{{ t('footer.privacy') }}</NuxtLink>
    </div>
  </footer>
</template>
