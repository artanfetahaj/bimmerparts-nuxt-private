...
const validateForm = () => {
  ...
  requiredFields.forEach(field => {
    const value = formData.value[field as keyof typeof formData.value]
    if (!value || value.trim() === '') errors[field] = t('contact.fieldRequired')
  })
  if (formData.value.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.value.email)) {
    errors.email = t('contact.validEmail')
  }
  if (!acceptPrivacy.value) errors.privacy = t('contact.agreePrivacy')
  ...
  <div class="min-h-screen bg-white">
    <div class="container mx-auto px-6 py-16">
      <div class="text-center mb-16">
        ...
        <p class="text-lg text-gray-600 max-w-2xl mx-auto">{{ t('contact.subtitle') }}</p>
      </div>
          ...
          <div class="border border-gray-200 rounded-lg p-6 h-full flex flex-col">
            ...
            <form @submit.prevent="handleSubmit" class="space-y-6 flex-1 flex flex-col">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('contact.firstName') }} <span class="text-red-500">*</span></label>
                  <input v-model="formData.firstName" @input="validationErrors.firstName = ''" type="text" class="w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" :class="validationErrors.firstName ? 'border-red-500' : 'border-gray-300'" />
                  <p v-if="validationErrors.firstName" class="text-red-500 text-sm mt-1">{{ validationErrors.firstName }}</p>
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('contact.lastName') }} <span class="text-red-500">*</span></label>
                  <input v-model="formData.lastName" @input="validationErrors.lastName = ''" type="text" class="w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" :class="validationErrors.lastName ? 'border-red-500' : 'border-gray-300'" />
              ...
              <div>
                ...
                <input v-model="formData.email" @input="validationErrors.email = ''" type="email" class="w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" :class="validationErrors.email ? 'border-red-500' : 'border-gray-300'" />
                <p v-if="validationErrors.email" class="text-red-500 text-sm mt-1">{{ validationErrors.email }}</p>
              </div>

              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('contact.phoneNumber') }} <span class="text-red-500">*</span></label>
                <input v-model="formData.phone" @input="validationErrors.phone = ''" type="tel" class="w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" :class="validationErrors.phone ? 'border-red-500' : 'border-gray-300'" />
                <p v-if="validationErrors.phone" class="text-red-500 text-sm mt-1">{{ validationErrors.phone }}</p>
              </div>

              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('contact.message') }} <span class="text-red-500">*</span></label>
                <textarea v-model="formData.message" @input="validationErrors.message = ''" rows="4" :placeholder="t('contact.messagePlaceholder')" class="w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 resize-none" :class="validationErrors.message ? 'border-red-500' : 'border-gray-300'"></textarea>
              ...
              <div class="p-4 rounded-lg border-2 transition-colors" :class="validationErrors.privacy ? 'border-red-500 bg-red-50' : 'border-gray-200 bg-gray-50'">
                <label class="flex items-start space-x-3 cursor-pointer">
                  ...
                  <span class="text-sm text-gray-700 font-medium">
                    ...
                    <NuxtLink to="/terms" class="text-orange-500 hover:underline font-semibold">{{ t('contact.termsAndConditions') }}</NuxtLink>. <span class="text-red-500">*</span>
                  </span>
                </label>
              ...
              <button
                ...
                :class="acceptPrivacy ? 'bg-orange-500 text-white hover:bg-orange-600 cursor-pointer' : 'bg-gray-300 text-gray-500 cursor-not-allowed opacity-60'"
              >
